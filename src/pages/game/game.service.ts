...
export class GameService{
    ...
    delete(obj){
        this.deleteGameById(obj.objectId).subscribe(data=>{
            console.log(data);
    ...
    deleteGameById(objectId):Observable<any>{
            // 1. 拼接HTTP请求所需的URL和Headers
            let serverURL = "http://host.qh-class.com:2337/parse"
            let path = "/classes/"
            let className = "Game"
    ...
    getGameById(objectId):Observable<any>{
        // 1. 拼接HTTP请求所需的URL和Headers
        let serverURL = "http://host.qh-class.com:2337/parse"
        let path = "/classes/"
        let className = "Game"
    ...
    saveGame(body?):Observable<any[]>{
        ...
        if(!body){
            body = {name:"MyGame",platform:"Android",review:"23333",rating:"5"}
        }
